...
const CREATOR_SIG_LIMIT = Number(process.env.RUG_FORENSICS_CREATOR_SIG_LIMIT || "40");
...
async function onChainProbe(connection, event) {
  ...
  try {
    const account = await connection.getAccountInfo(new PublicKey(event.pool), "confirmed");
  ...
  try {
    const sigs = await connection.getSignaturesForAddress(new PublicKey(event.pool), { limit: 300 }, "confirmed");
...
async function main() {
  ...
  const analyzed = [];
  for (const event of rugLossEvents) {
