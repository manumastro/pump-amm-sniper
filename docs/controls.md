...
| `STONK_USCITE_TEMPO` | 5,10,30 s | uscite **a tempo**, che non aspettano nessun obiettivo. FiFawHqx sta dentro 4 secondi di mediana e il suo caso peggiore su 55 giri e' **-4,7%**: a quella velocita' lo stop non serve, si esce prima che la curva possa svuotarsi. Il nostro stop a -10% ci fa uscire fra -15% e -21% per lo scivolamento, ed e' li' che se ne va il conto |
...
| `STONK_USCITE_PAREGGIO` | 5,10,30 s | uscite a **pareggio ritardato**: dopo N secondi si esce appena non si e' in guadagno. Misurato: le vincite arrivano in ~6 secondi mediani, le perdite marciscono per 24-89, e solo l'8% delle perdite si chiude entro 5 secondi contro il 46% delle vincite. Uscire *sempre* a N secondi invece non paga — sulle stesse pool `t5` salva 10 punti sulle perdenti ma ne butta 16 sulle vincenti, e infatti fa -5,4% |
