...
| `STONK_COMPRA_MINIMA` | 0,005 | sopra questa quota del bersaglio si va a vedere **chi** ha comprato (`scripts/stonk-compratori.js`), sotto si conta e basta. La dimensione dell'acquisto si legge dall'evento di LaunchLab a costo zero; il portafoglio sta nella transazione e costa una chiamata, quindi la soglia serve solo a decidere per chi spenderla |
| `STONK_NOMI_AL_SEC` | 5 | quanti nomi di compratori risolvere al secondo |
| `STONK_SALTO_MINIMO` | 0,01 | il **segnale**: si compra subito dopo un acquisto che ha spostato la curva di almeno 1 punto di raccolta in un solo aggiornamento. Nasce dal fallimento della soglia: comprando in un momento qualsiasi dopo l'1,5%, 165 chiusure su 367 sono arrivate a scadenza col prezzo **fermo esattamente dov'era**, pagando 5,4% (tassa 1%) o 9,2% (tassa 3%) di costi per niente. FiFawHqx muove 1,75 punti a ogni acquisto: il valore e' preso da li' |
| `STONK_SALTO_MAX_INGRESSO` | 0,10 | tetto suo per il canale del salto: un salto puo' far atterrare la curva ben oltre il 2,5% della soglia, e rifiutarlo per quello vanificherebbe il segnale |
